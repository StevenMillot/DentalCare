/**
 * Carrousel média cabinet — logique principale.
 *
 * Fonctions : layout infini (clones), autoplay, swipe, miniatures, lightbox (clavier + flèches).
 * Appelé par `index.js` ; ne pas charger directement dans le HTML.
 *
 * @param {HTMLElement} root — `#cabinet-media-carousel`
 ...
 * @returns {{ destroy: function }|null}
 */
export function initCabinetMediaCarousel(root) {
    if (!root) return null;
    ...
    /** Pause autoplay au survol de la diapositive centrale */
    let hoverPause = false;
    /** Pause temporaire après navigation manuelle (flèches) */
    ...
    originals.forEach((fig, i) => {
      ...
      if (fig.querySelector('video')) {
        ...
        playBadge.innerHTML =
          ...
          `<path d="${CONFIG.PLAY_ICON_PATH}"/></svg>`;
    ...
    /**
     * @param {{ runFirstTickNow?: boolean }} [opts]
     * runFirstTickNow — après une pause interaction, avance tout de suite au lieu d’attendre un intervalle complet.
     */
    ...
    /** Reprend l’autoplay après un délai (même si le curseur reste sur les flèches). */
    ...
    function updateLightboxMeta(fig, ri) {
      goToRealIndex(ri);
      if (lightboxEl) {
        const label = fig.getAttribute('data-label') || `Média ${ri + 1}`;
    ...
    function setLightboxContent(ri, opts = {}) {
      ...
      const animate =
        ...
        lightboxMediaHost &&
        lightboxMediaHost.firstElementChild;
    ...
    function ensureLightbox() {
      ...
      lightboxEl.innerHTML =
        ...
        '<div class="cabinet-lightbox__nav-bar"></div>' +
        '</div>';
    ...
    function openLightboxFromSlide(fig) {
      ...
      if (ri < 0) return;
      const hasMedia =
        fig.querySelector('.cabinet-media-carousel__media img') ||
        fig.querySelector('.cabinet-media-carousel__media video');
      if (!hasMedia) return;
